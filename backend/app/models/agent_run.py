...
记录：Planner 计划、节点执行明细、Token 消耗与错误。
"""
...
class AgentRun(Base):
    ...
    生命周期状态：pending → running → succeeded / failed / cancelled。

    当前实现不包含 checkpoint resume 或人工审批状态机。
    """
    ...
    # 历史原型列：保留 ORM/既有迁移一致性，但当前 API 与工作流均不读写，
    # 不能据此声称实现了 Tool Calling、checkpoint resume 或人工审批。

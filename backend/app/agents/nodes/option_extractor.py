...
class OptionExtractionError(RuntimeError):
    """OptionExtractor 调用或结构化解析失败。"""
...
async def extract_options(
    ...
    Returns:
        结构化决策对象；调用或解析失败抛出 OptionExtractionError
    """
    ...
    user_prompt = EXTRACTOR_PROMPT.format(
    ...
    try:
        ...
        raise OptionExtractionError(f"LLM 调用失败: {e}") from e
    ...
    try:
        ...
        raise OptionExtractionError(f"JSON/Schema 解析失败: {e}") from e

    # 校验：chosen 必须等于 options 中某个 name
    option_names = [opt.name for opt in decision.options]
    if decision.chosen not in option_names:
        raise OptionExtractionError(
            f"chosen={decision.chosen!r} 不在候选方案 {option_names} 中"
        )

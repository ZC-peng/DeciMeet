...
function ChatMessageVirtualListBase({
  ...
  const { parentRef, virtualizer, items, totalSize, scrollToBottom, isNearBottom } =
    useVirtualList({
      count,
      estimateSize: 120,
      overscan: 4,
  ...
  useEffect(() => {
    if (count > prevCount.current && isNearBottom) {
      scrollToBottom()
    }
    prevCount.current = count
  }, [count, isNearBottom, scrollToBottom])
  ...
  }, [streamingContent, hasStreaming, isNearBottom, scrollToBottom])

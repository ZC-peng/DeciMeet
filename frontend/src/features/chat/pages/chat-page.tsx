...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    if ((!input.trim() && images.length === 0) || !currentSessionId || isStreaming) return

    const query = input.trim() || '请分析我上传的图片。'
              ...
              {localMessages.length > 0 &&
                ...
                !isStreaming && (
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => handleToggleTTS(localMessages[localMessages.length - 1].content)}

...
export default function DecisionDetailPage() {
  ...
  const { data: decision, isLoading, isError, error, refetch } = useDecisionDetail(id)
  ...
  if (isError && !isNotFoundError(error)) {
    return (
      <div className="space-y-6">
        ...
        <QueryErrorState error={error} title="决策详情加载失败" onRetry={() => void refetch()} />
      </div>
    )
  }
            ...
            {decision.related_decisions.map((rel) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  {rel.context && (
                    <p className="line-clamp-1 text-xs text-muted-foreground">{rel.context}</p>
                  )}
                </div>
